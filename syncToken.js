...
async function ensureV2Pool(db, provider, lpAddress) {
  ...
  await db.query(`
    INSERT INTO token_stats (
      contract_address, parent_token_address, token_name, token_symbol, decimals, total_supply, transfer_count,
      is_lp, lp_version, token0_address, token1_address, token0_balance, token1_balance
    ) VALUES (?, ?, ?, ?, ?, ?, 0, 1, 'V2', ?, ?, ?, ?)
  `, [
    lpAddress, // LP 合约地址
    parentToken, // ✅ 母币地址
    parentInfo.name || 'Unknown', // 母币名称
    parentInfo.symbol || 'UNK', // 母币符号
    parentInfo.decimals || 18, // 母币精度
    parentInfo.totalSupplyReadable || '0', // 母币发行量
    v2Info.token0,
    v2Info.token1,
    token0_balance,
    token1_balance
...
async function updateTradeVolume(db, lpAddress, usdtVolume) {
  try {
    await db.query(`
      UPDATE token_stats
      SET transaction_volume = transaction_volume + ?, 
          transaction_volume_12h = transaction_volume_12h + ?,
          trade_count_24h = trade_count_24h + 1
      WHERE contract_address = ?
    `, [usdtVolume, usdtVolume, lpAddress]);
  } catch (err) {
